...
const DisplayAlbum = () => {
  const { id } = useParams();
  const albumsData1 = albumsData[id];
  const {playWithId} = useContext(PlayerContext)
  return (
    <>
      <Navbar />
      <div className="mt-10 flex gap-8 flex-col md:flex-row md:items-end">
        <img className="w-48 rounded" src={albumsData1.image} alt="" />
        <div className="flex flex-col">
          <p>Playlist</p>
          <h2 className="text-5xl font-bold mb-4 md:text-7xl">
            {albumsData1.name}
          </h2>
          <h4>{albumsData1.desc}</h4>
          <p className="mt-1 ">
            <img className="inline-block w-5" src={assets.spotify_logo} alt="" />
            <b> Saitify</b> 14,23,413 likes |
            <b> 50 songs </b>| about 2 hr 35 min
          </p>
        </div>
      </div>
      ...
      <hr />
      {
        songsData.map((item,index)=>(
            <div onClick={()=>playWithId(item.id)} key={index} className="grid grid-cols-3 sm:grid-cols-4 gap-3 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer">
                ...
                <p className="text-[15px]">{albumsData1.name}</p>
